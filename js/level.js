...
class Level {
	...
	generateLevel(seed = getRandomInt(0, 1000000), prevLevelPart = null) {
		...
		while (tempCounter < 10) {
			...
			if (!prevLevelPart) {
				...
				prevLevelPart = newLevelPart.generateLevelPart(0)
				this.levelParts.push(prevLevelPart)
				++tempCounter
	...
	filterGrounds(position, radius) {
		...
		return grounds.filter((currentGround) => {
			...
			return (
				currentGround.boundingBox.min.x < position.x &&
				position.x < currentGround.boundingBox.max.x &&
				currentGround.boundingBox.min.z < position.z &&
				position.z < currentGround.boundingBox.max.z &&
